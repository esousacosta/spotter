...
export function EditTradeForm({ trade, onSuccess, onCancel }: EditTradeFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    contractMultiplier: String(trade.contractMultiplier),
    entryNetDebit: String(trade.entryNetDebit),
    entryCommissions: String(trade.entryCommissions),
    edgeAtEntry: String(trade.edgeAtEntry ?? 0),
    notes: trade.notes ?? '',
    legs: trade.legs.map(leg => ({
      id: leg.id as string | undefined,
      side: leg.side,
    ...
    <form onSubmit={handleSubmit} className="trade-form">
      ...
      <div>
        ...
        {formData.legs.map((leg, idx) => (
          <div key={leg.id ?? `new-${idx}`} className="leg-card">
